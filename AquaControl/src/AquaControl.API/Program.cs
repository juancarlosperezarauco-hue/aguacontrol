...
builder.Services.AddDbContext<AquaDb>(o=>o.UseSqlServer(connection,s=>s.UseNetTopologySuite()));builder.Services.AddScoped<IData>(s=>s.GetRequiredService<AquaDb>());builder.Services.AddScoped<AquaService>();builder.Services.AddScoped<CatalogService>();builder.Services.AddScoped<SecurityService>();builder.Services.AddScoped<GeoService>();builder.Services.AddSingleton<IPaymentGateway>(new PaymentGateway(sandbox));
